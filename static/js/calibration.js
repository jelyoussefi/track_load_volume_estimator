...
window.CalibrationManager = {
    ...
    startCalibration(cameraId) {
        ...
        info.textContent = `Click exactly ${requiredPoints} corners to create calibration rectangle.`;
        info.classList.add('show');
        
        if (window.Utils) {
            window.Utils.showNotification(`Calibration started for Camera ${cameraId}. Click exactly ${requiredPoints} corners.`, 'info');
        }
    },
    ...
    resetCalibration() {
        if (confirm('Reset all calibration data? This cannot be undone.')) {
            ...
            // Hide info displays
            if (window.Utils) {
                window.Utils.toggleElementClass('calibrationInfo1', 'show', false);
                window.Utils.toggleElementClass('calibrationInfo2', 'show', false);
            } else {
                const info1 = document.getElementById('calibrationInfo1');
                const info2 = document.getElementById('calibrationInfo2');
                if (info1) info1.classList.remove('show');
                if (info2) info2.classList.remove('show');
            }
...
const CalibrationManager = {
    ...
    setupCanvases() {
        for (let cameraId = 1; cameraId <= 2; cameraId++) {
            ...
            img.addEventListener('load', resizeCanvasForCamera);
            img.addEventListener('loadeddata', resizeCanvasForCamera);
            window.addEventListener('resize', Utils.debounce(resizeCanvasForCamera, 100));
            ...
            // Initial resize
            setTimeout(resizeCanvasForCamera, CONFIG.TIMING.CANVAS_RESIZE_DELAY);
        }
    },
    ...
    resizeCanvas(cameraId) {
        ...
        if (img && canvas && img.naturalWidth && img.naturalHeight) {
            ...
            setTimeout(() => {
                this.redrawCalibrationPolygon(cameraId);
            }, CONFIG.TIMING.CANVAS_RESIZE_DELAY);
    ...
    startCalibration(cameraId) {
        ...
        // Pause video processing
        processingManager.pauseForCalibration();
        ...
        // Show instruction for exactly 4 corners
        info.textContent = `Click exactly ${CONFIG.CALIBRATION.REQUIRED_POINTS} corners to create calibration rectangle.`;
        info.classList.add('show');
        
        Utils.showNotification(`Calibration started for Camera ${cameraId}. Click exactly ${CONFIG.CALIBRATION.REQUIRED_POINTS} corners.`, 'info');
    },
    ...
    handleCanvasClick(event, cameraId) {
        ...
        // Stop if already have required points
        if (calibData.points.length >= CONFIG.CALIBRATION.REQUIRED_POINTS) {
            Utils.showNotification('Rectangle complete. Collecting dimensions...', 'info');
            return;
        }
        ...
        console.log(`Added point ${calibData.points.length}/${CONFIG.CALIBRATION.REQUIRED_POINTS} for camera ${cameraId}:`, { x: scaledX, y: scaledY });
    },
    ...
    drawCalibrationPoint(canvas, x, y, pointNumber) {
        ...
        // Draw point
        ctx.fillStyle = CONFIG.COLORS.POINT_COLOR;
        ctx.beginPath();
        ctx.arc(x, y, CONFIG.CALIBRATION.POINT_RADIUS, 0, 2 * Math.PI);
        ctx.fill();
        
        // Add point number with white outline for better visibility
        ctx.strokeStyle = CONFIG.COLORS.TEXT_OUTLINE;
        ctx.lineWidth = 3;
        ctx.font = 'bold 14px Arial';
        ctx.textAlign = 'center';
        ctx.strokeText(pointNumber.toString(), x, y + 5);
        
        ctx.fillStyle = CONFIG.COLORS.TEXT_FILL;
        ctx.fillText(pointNumber.toString(), x, y + 5);
        
        // Draw line to previous point
        if (calibData.points.length > 1) {
            const prevPoint = calibData.points[calibData.points.length - 2];
            ctx.strokeStyle = CONFIG.COLORS.LINE_COLOR;
            ctx.lineWidth = CONFIG.CALIBRATION.LINE_WIDTH;
            ctx.beginPath();
            ctx.moveTo(prevPoint.x, prevPoint.y);
            ctx.lineTo(x, y);
            ctx.stroke();
    ...
    completeCalibrationShape(cameraId) {
        ...
        // Close the rectangle
        const firstPoint = calibData.points[0];
        const lastPoint = calibData.points[CONFIG.CALIBRATION.REQUIRED_POINTS - 1];
        ...
        // Fill the rectangle with transparent color
        ctx.fillStyle = CONFIG.COLORS.FILL_COLOR;
        ctx.beginPath();
        ctx.moveTo(calibData.points[0].x, calibData.points[0].y);
        for (let i = 1; i < calibData.points.length; i++) {
            ctx.lineTo(calibData.points[i].x, calibData.points[i].y);
        ...
        // Resume video processing
        processingManager.resumeAfterCalibration();
        ...
        Utils.showNotification(`Rectangle complete for Camera ${cameraId}. Starting dimension collection...`, 'success');
    },
    ...
    updateCalibrationInfo(cameraId) {
        ...
        if (calibData.points.length < CONFIG.CALIBRATION.REQUIRED_POINTS) {
            info.textContent = `Point ${calibData.points.length}/${CONFIG.CALIBRATION.REQUIRED_POINTS} added. ${CONFIG.CALIBRATION.REQUIRED_POINTS - calibData.points.length} more needed.`;
        } else {
            info.textContent = 'Rectangle complete! Starting dimension collection...';
        }
    },
    ...
    collectDimensions(cameraId) {
        ...
        if (points.length !== CONFIG.CALIBRATION.REQUIRED_POINTS) {
            Utils.showNotification(`Need exactly ${CONFIG.CALIBRATION.REQUIRED_POINTS} points for calibration`, 'error');
            return;
        }
    ...
    promptForDimensions(cameraId, edges, edgeIndex) {
        ...
        if (!Utils.isValidPositiveNumber(dimension)) {
    ...
    calculateCalibration(cameraId) {
        ...
        if (distances.length === 0) {
            Utils.showNotification('No dimensions provided', 'error');
            return;
        }
        ...
        if (validCount === 0) {
            Utils.showNotification('No valid calibration data', 'error');
            return;
        }
        ...
        // Calculate area in square meters
        const areaPixels = Utils.calculatePolygonArea(calibData.points);
        const areaSquareMeters = areaPixels / (calibData.pixelsPerMeter * calibData.pixelsPerMeter);
        
        // Update display
        const info = document.getElementById(`calibrationInfo${cameraId}`);
        info.textContent = `Calibrated: ${Utils.formatNumber(areaSquareMeters)} m² | ${Utils.formatNumber(calibData.pixelsPerMeter, 1)} px/m`;
        info.classList.add('show');
        ...
        Utils.showNotification(`Camera ${cameraId} calibrated successfully!`, 'success');
    },
    ...
    clearCalibration(cameraId) {
        ...
        console.log(`Cleared calibration for camera ${cameraId}`);
        Utils.showNotification(`Camera ${cameraId} calibration cleared`, 'info');
    },
    ...
    redrawCalibrationPolygon(cameraId) {
        ...
        if (calibData.points.length === CONFIG.CALIBRATION.REQUIRED_POINTS && calibData.pixelsPerMeter) {
            const ctx = canvas.getContext('2d');
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            ...
            calibData.points.forEach((point, index) => {
                ...
                ctx.fillStyle = CONFIG.COLORS.POINT_COLOR;
                ctx.beginPath();
                ctx.arc(point.x, point.y, CONFIG.CALIBRATION.POINT_RADIUS, 0, 2 * Math.PI);
                ctx.fill();
    ...
    saveCalibrationData() {
        try {
            localStorage.setItem(CONFIG.CALIBRATION.STORAGE_KEY, JSON.stringify(this.calibrationData));
        } catch (e) {
            console.warn('Could not save calibration data to localStorage:', e);
        }
    },
    ...
    loadCalibrationData() {
        try {
            const saved = localStorage.getItem(CONFIG.CALIBRATION.STORAGE_KEY);
            if (saved) {
                const loaded = JSON.parse(saved);
                // Merge with current data (excluding isCalibrating state)
                for (let key in loaded) {
                ...
                setTimeout(() => {
                    this.redrawCalibrationPolygon(1);
                    this.redrawCalibrationPolygon(2);
                }, CONFIG.TIMING.CALIBRATION_REDRAW_DELAY);
            }
        } catch (e) {
            console.warn('Could not load calibration data from localStorage:', e);
        }
    ...
    exportCalibration() {
        ...
        Utils.showNotification('Calibration data exported', 'success');
    },
            ...
            if (file) {
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        Utils.showNotification('Calibration data imported successfully', 'success');
                        console.log('Imported calibration data:', this.calibrationData);
                    } catch (error) {
                        Utils.showNotification('Failed to import calibration data', 'error');
                        console.error('Import error:', error);
                    }
                };
                reader.readAsText(file);
    ...
    resetCalibration() {
        if (confirm('Reset all calibration data? This cannot be undone.')) {
            ...
            // Hide info displays
            Utils.toggleElementClass('calibrationInfo1', 'show', false);
            Utils.toggleElementClass('calibrationInfo2', 'show', false);
            ...
            this.saveCalibrationData();
            Utils.showNotification('All calibration data reset', 'info');
            console.log('All calibration data reset');
        }
    },
    ...
    cancelActiveCalibrations() {
        for (let cameraId = 1; cameraId <= 2; cameraId++) {
            if (this.calibrationData[`camera${cameraId}`].isCalibrating) {
                this.clearCalibration(cameraId);
                Utils.showNotification(`Calibration cancelled for Camera ${cameraId}`, 'info');
            }
        }
    },
    ...
    isCalibrated(cameraId) {
        const calibData = this.calibrationData[`camera${cameraId}`];
        return !!(calibData.pixelsPerMeter && calibData.points.length === CONFIG.CALIBRATION.REQUIRED_POINTS);
    }
};